...
public class ServiceImplementation implements Serviceclass
{
	...
	public ServiceResponse postDetails(UserDto dto)
	{
		...
		try
		{
			...
			repo.save(entity);
			return new ServiceResponse("success","Data Inserted Suceessfully",null);
		}
		catch(Exception e)
		{
			logger.error("Error:" + e.getMessage(), e);
			return new ServiceResponse("fail","Exception Occured",null);
		}
	...
	public ServiceResponse updateDetails(UserDto dto)
	{
		...
		try
		{
			...
			repo.save(entity);
			return new ServiceResponse("success","Updated Successfully",null);
		}
		catch(Exception e)
		{
			logger.error("Error:" + e.getMessage(), e);
			return new ServiceResponse("failed","Exception Occured",null);
		}
	...
	public ServiceResponse verifyDetails(IdDto id)
	{
		...
		try
		{
			Optional<EntityClass> num=repo.findById(entityid);
			EntityClass entity=num.get();
			boolean check=checktoverify(entity.getModifiedby(),entity.getCreatedby(),id.getCurrentuser());
			if(check)
			{
				return new ServiceResponse("failure","Created / Modified User Cannot Verify",null);
			}
			else
			{
				entity.setStatus("VERIFIED");
				entity.setModifiedby(id.getCurrentuser());
				entity.setMdate(currentDate);
				repo.save(entity);
				return new ServiceResponse("success","Verified Successfullt",null);
			}
	...
	public ServiceResponse deleteDetails(IdDto id)
	{
		...
		try
		{
			...
			entity.setMdate(currentDate);
			repo.save(entity);
			return new ServiceResponse("success","Deleted Successfully",null);
		}
		catch(Exception e)
		{
			logger.error("Error:" + e.getMessage(), e);
	...
	private String user()
	{
	 ...
	 private boolean checktoverify(String str1,String str2,String str3)
	  {
		  if ((str1 != null && str1.equals(str3)) || (str2 != null && str2.equals(str3))) {
		        return true;
		    } else {
		        return false;
		    }
